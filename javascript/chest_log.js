...
function loadChestRewardCounter() {
  ...
  header.innerHTML = `<th>#</th><th>ChestId</th><th>Reward</th><th>Qty</th><th>Count</th>`;
  ...
  for (const item of rewardData) {
    ...
    if (item.reward.includes("scroll")) {
      tr.innerHTML = `<td>${counter}</td><td>${item.chestId}</td><td><div class="crop"><img src="${item.url}" title="${item.reward}"></div></td><td>x${qty}</td><td>${item.count}</td>`;
    } else {
      tr.innerHTML = `<td>${counter}</td><td>${item.chestId}</td><td><img src="${item.url}" title="${item.reward}" style="height: 30px; width: auto"></td><td>x${qty}</td><td>${item.count}</td>`;
    }
